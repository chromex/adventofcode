...
def foo(input):
    lst = list(map(input.find, names))
    #print(lst)
    result = []
    for i, x in enumerate(lst):
        if (x >= 0):
            result.append((x, i + 1))
    lst = list(map(input.rfind, names))
    #print(lst)
    for i, x in enumerate(lst):
        if (x >= 0):
            result.append((x, i + 1))
    #print(result)
    fstDig = re.search(r"\d", input)
    if fstDig != None:
        result.append((fstDig.start(), int(input[fstDig.start()])))
    lstDig = re.search(r"\d", input[::-1])
    if lstDig != None:
        idx = len(input) - lstDig.start() - 1
        result.append((idx, int(input[idx])))
    return list(map(lambda x: x[1], sorted(result, key=lambda x: x[0])))


#print(foo("tdszrfzspthree2ttzseven5seven"))
